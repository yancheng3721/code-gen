...
public class FeatureManualSortConfig implements ModuleConfig {
	...
	public LinkedHashMap<String, Map<String, String>> getAllFileds() {
		...
		results.put("KEYWORD",
				CodeGenerator.newField("KEYWORD", "专题编码", "NOT_NULL"));
		results.put("NAME", CodeGenerator.newField("NAME", "专题名称"));
	...
	public LinkedHashMap<String, String> getAllReplaceMent() {
		...
		replacement
				.put("@querySql@",
						"SELECT * FROM (SELECT A.ID,A.KEYWORD,B.DOC_NAME AS NAME,A.TYPE,A.PARTNUMBER,A.SORT,A.START_TIME,A.END_TIME "
								+ "FROM EB_MANUAL_INTERVENTION_SORT AS A LEFT JOIN TF_DEFINITION_DOC AS B ON B.DOC_CODE = A.KEYWORD) "
								+ "WHERE TYPE =3 ORDER BY KEYWORD ASC");
		return replacement;
	}
